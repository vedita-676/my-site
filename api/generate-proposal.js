...
// Optional tool — only available if Telegram is configured
const ALERT_OWNER_TOOL = {
  type: 'function',
  function: {
    name: 'alert_owner',
...
// Build tools list — optional tools only included when their env vars are configured
function getTools() {
  const tools = [...CORE_TOOLS];
  if (process.env.TELEGRAM_BOT_TOKEN && process.env.TELEGRAM_CHAT_ID) {
    tools.push(ALERT_OWNER_TOOL);
  }
...
async function renderProposalPdf({ company_name, contact_name, sections }) {
  ...
  cover.drawText('StepChange', {
    x: 50, y: 732, size: 22, font: fontBold, color: rgb(1, 1, 1),
  });
  cover.drawText('Climate risk & ESG intelligence for financial institutions', {
...
2. ESG reporting & carbon accounting — end-to-end BRSR, TCFD, and voluntary disclosure support for banks and large enterprises.
3. Parametric insurance design — helping banks design climate peril-based parametric insurance products for customers with high climate risk exposure.
4. Advisory & consulting — bespoke engagements on climate risk strategy, regulatory readiness, and sustainability data infrastructure.

## LEAD TRIAGE RULES
HIGH — Financial institution (bank, NBFC, insurer) with a live regulatory deadline, a direct ask around climate risk or ESG reporting, or a budget signal above INR 20L.
MEDIUM — Enterprise or financial institution exploring climate risk or ESG, no hard deadline, moderate budget signal.
LOW — Individual, student, researcher, or non-FI company with no clear commercial intent.

## PROPOSAL STRUCTURE
Write 4-5 sections:
1. Understanding Your Challenge — show you listened to their specific situation; reference what they said
2. Why StepChange — what makes StepChange specifically right for this, given their context
3. Recommended Approach — concrete, scoped to their problem
4. Proposed Engagement — which service, scope, timeline (typical engagements run 3-6 months)
...
- Call alert_owner with: company, contact name, challenge summary, lead score, and one sentence on why you scored it that way
